...
*   **No Direct .agents Modifications:** Không được sửa đổi trực tiếp các file/thư mục trong `.agents/`. Khi cần chỉnh sửa tài nguyên hoặc mã nguồn của skill, bạn bắt buộc phải thực hiện sửa đổi trong thư mục `shared/`, sau đó tiến hành `git commit`, `git push`, và chạy lệnh `kit update` để cập nhật lại thư mục `.agents/`.
