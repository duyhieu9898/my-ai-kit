            ...
            for pattern in patterns:
                ...
                if matches and not has_i18n:
                    ...
                    if len(hardcoded_examples) < 5:
                        ...
                        hardcoded_examples.append(f"{rel_path}: {str(matches[0])[:40]}...")
...
def main():
    ...
    has_i18n_infrastructure = len(locale_files) > 0
    ...
    for item in code_result['issues']:
        # If no i18n infrastructure exists, hardcoded strings are expected
        # behavior for a monolingual project — downgrade from critical to warning
        if not has_i18n_infrastructure and item.startswith("[X]"):
            item = "[!]" + item[3:]
        print(f"  {item}")

    # Summary
    # Only count critical issues; for projects without i18n infrastructure,
    # hardcoded strings are warnings, not blockers
    all_issues = locale_result['issues'] + code_result['issues']
    if has_i18n_infrastructure:
        critical_issues = sum(1 for i in all_issues if i.startswith("[X]"))
    else:
        # Without locale infrastructure, only locale-level [X] issues are critical
        critical_issues = sum(1 for i in locale_result['issues'] if i.startswith("[X]"))
